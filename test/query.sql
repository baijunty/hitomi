...
SELECT g.* FROM Gallery g,json_each(g.artist) ja  where (json_valid(g.artist)=1 and ja.value = 'ichinomiya yuu') 
SELECT * from Tasks WHERE completed=0
SELECT * FROM Tags WHERE translate='momota tetsu'
select count(1) from Gallery g 
select * from GalleryFile where name='125_pg_122_1.06_MB.png'
...
select * from GalleryFile where gid=2792465  order by name
