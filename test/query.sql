...
SELECT g.* FROM Gallery g,json_each(g.artist) ja  where (json_valid(g.artist)=1 and ja.value = 'charu') 
SELECT * from Tasks WHERE completed=0
SELECT * FROM Tasks WHERE id=668271
...
select * from Gallery g where g.id=668271
