...
class DownLoader {
  ...
  Future<bool> messageHandle(Message msg) async {
    ...
    switch (msg) {
          ...
          if (msg.target is List) {
            if (msg.success) {
              ...
              await helper.updateTask(
                ...
                msg.gallery.dirName,
                msg.file.path,
                (msg.target as List<Image>).length / msg.gallery.files.length >
                    0.1,
              );
            }
            ...
            return helper
                .querySql('select * from GalleryFile where gid=? and name=?', [
                  msg.gallery.id,
      ...
      case DownLoadingMessage():
        {
          var key = _runningTask.keys.firstWhereOrNull(
            (element) => element.gallery.id == msg.id,
          );
          if ((key != null)) {
  ...
  Future<bool> _findUnCompleteGallery(Gallery gallery, Directory newDir) async {
    ...
    return Future.wait(ids.map((id) => helper.queryGalleryById(id))).then((
      ...
      var target = compareGallerWithOther(
        gallery,
        list,
        config.languages,
        logger,
  ...
  bool illeagalTagsCheck(Gallery gallery, List<List<FilterLabel>> excludes) {
    final labels = gallery.labels();
    if (labels.isEmpty) {
      return true;
    }
    var exits = excludes.where(
      (elements) => elements.every((e) => labels.contains(e)),
    );
    if (exits.isNotEmpty) {
      logger?.d(
        '${gallery.id} illegal tags ${exits.map((e) => e.map((e) => e.name).join(' ')).toList()}',
      );
    }
  ...
  Future<bool> cancelById(int id) async {
    var target = _runningTask.keys.firstWhereOrNull(
      (element) => element.gallery.id == id,
    );
    if (target != null) {
  ...
  Future<bool> deleteById(int id) async {
    ...
    if (target == null) {
      var path = await helper.readlData<String>('Gallery', 'path', {
        'id': id,
      }); // 读取路径数据
      if (path != null) {
    ...
    if (target != null && target.id == id) {
      await HitomiDir(
        target.createDir(config.output, createDir: false),
        this,
        target,
  ...
  Future<bool> notifyTaskChange({int? id}) async {
    if (_runningTask.length < min(5, config.maxTasks) &&
      ...
      IdentifyToken? token;
      if (id != null) {
        token = _pendingTask.firstWhereOrNull(
          (element) => element.gallery.id == id,
        );
      }
      token = token ?? _pendingTask.first;
      _pendingTask.removeWhere(
        (element) => element.gallery.id == token!.gallery.id,
      );
      taskObserver({
        'id': token.gallery.id,
        'type': 'remove',
    ...
    if (ids.isNotEmpty) {
      ...
      return helper
          .selectSqlMultiResultAsync(
            'select id,path from Gallery where id =?',
            ids.map((e) => [e]).toList(),
          )
          .then(
            (value) => Stream.fromIterable(value.entries)
                .asyncMap((event) async {
                  try {
                    String? path = event.value.firstOrNull?['path'];
                    var fromNet = api.fetchGallery(
                      event.key[0],
                      usePrefence: false,
                      token: token,
                    );
                    ...
                    (pre) =>
                        pre.languages?.any(
                              (lang) =>
                                  ...
                                  max(
                                        0,
                                        config.languages.indexOf(pre.language!),
                                      ) <=
                                      max(
                                        0,
                                        config.languages.indexOf(
                                          element.language!,
                                        ),
                                      ),
                            ) ==
  ...
  Future<List<Gallery>> _filterGalleryByImageHash(
    List<Gallery> list,
    CancelToken token,
    ...
    if (entry != null) {
      ...
      return list
          ...
          .where(
            (event) => searchSimilerGaller(
              MapEntry(event.key.id, event.value),
              allHash,
              logger: logger,
              threshold: config.threshold,
            ).isEmpty,
          ...
          .fold(<int, List<int>>{}, (previous, element) {
            var duplicate = searchSimilerGaller(
              MapEntry(element.key.id, element.value),
              previous,
              logger: logger,
              threshold: config.threshold,
            );
          ...
          .then(
            (value) => value
                .map(
                  (event) => list.firstWhere((element) => element.id == event),
                )
                .toList(),
          )
          .catchError((err) {
  ...
  Future<List<Gallery>> fetchGallerysByTags(
    ...
    return await api
        .search(
          tags,
          exclude: config.excludes.fold(
            <Label>[],
            (acc, l) => acc..addAll(l.map((e) => fromString(e.type, e.name))),
          ),
        )
