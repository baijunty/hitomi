...
class DirScanner {
  ...
  Future<Map<int, List<int>>> _findDupByLaber(String type, String name) async {
    return _helper
        ...
        .then(
          (value) => value.entries.fold(
            <int, List<int>>{},
            (previousValue, element) =>
                previousValue
                  ..[element.key] = searchSimilerGaller(
                    element,
                    value,
  ...
  Future<Gallery?> _parseFromDir(String dir) async {
    ...
    var id = await _helper
        .querySql('select id from Gallery where path = ?', [dir])
        .then((value) => value.firstOrNull?['path']);
    if (id != null) {
...
class HitomiDir {
  ...
  Future<bool> _removeIllegalFiles(List<Image> dbImages) async {
    ...
    return dir
        ...
        .fold(<String>[], (previous, element) => previous..add(element))
        .then((value) async {
          if ((value.isNotEmpty)) {
            _downLoader.logger?.w(
              'del ${dir.path} files ${value} from ${gallery}',
            );
            await Future.wait(
              value.map(
  ...
  Future<bool> _compareThenDel(List<HitomiDir> others) async {
    ...
    if (left.id == this.gallery.id && dir.existsSync()) {
      return others
          .asStream()
          .asyncMap(
            (event) => event.deleteGallery(reason: 'compare exists ${left.id}'),
          )
        ...
        .then((hashList) async {
          return Future.wait(
            lost
                ...
                .map(
                  (e) => _downLoader.helper.insertGalleryFile(
                    gallery,
                    e.key,
                    e.value,
                  ),
